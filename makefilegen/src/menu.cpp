...
void Menu::run(std::ofstream& fout)
{
	...
	//Print::script(fout, "./" + decl::path_run_folder + "process.exe");
	Print::script(fout, "./" + decl::path_run_folder + "incongen.exe");
	Print::script(fout, "./" + decl::path_run_folder + "simulate.exe");
	Print::script(fout, "zip -r run/output-old/$(TIMESTAMP).zip run/output/");
}
