...
{
	Print::cmd(fout, "all", {"folder_check", "compile_necessary", "run_program"});
...
void Menu::compile_necessary(std::ofstream& fout, const std::vector<std::string>& file_vec)
{
	std::vector<std::string> deps =	Menu::convert_vec(
		list_exe(file_vec),
		Menu::convert_to_exe
	);

	Print::cmd(fout, "compile_necessary", deps);

	Print::echo(fout, "Command executed = compile_necessary");
}

void Menu::force(std::ofstream& fout)
{
	Print::cmd(fout, "force", {"folder_check", "clear_build", "compile_necessary", "run_program"});
	Print::echo(fout, "Command executed = force");

}

void Menu::clear_build(std::ofstream& fout)
{
	Print::cmd(fout, "clear_build");
