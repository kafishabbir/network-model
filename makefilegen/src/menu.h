...
public:
	...
	static void default_script(std::ofstream& fout);
	static void folder_check(std::ofstream& fout);
	static void compile_necessary(std::ofstream& fout, const std::vector<std::string>& file_vec);
	static void run_program(std::ofstream& fout);
	static void force(std::ofstream& fout);
	static void clear_build(std::ofstream& fout);
